...
class MeritzResidualExtractor:
    ...
    def _extract_vehicles_with_residuals(self, residual_tables: Dict,
        ...
        for row_idx in range(7, self.vehicle_sheet.max_row + 1):
            ...
            premium_add_15k = row_data[15] if len(row_data) > 15 else None  # 고잔가추가 15,000 (컬럼 P = 16)
            premium_add_10k = row_data[16] if len(row_data) > 16 else None  # 고잔가추가1 10,000 (컬럼 Q = 17)
            ...
            if west_grade:
                ...
                if west_normal:
                    ...
                    # 고잔가 = 일반잔가 + 8% + 장기계약 추가 (등급별 계수 적용)
                    residual_data['west_premium'] = self._apply_premium_adjustment(
                        west_normal, 0.08, long_term_premium, str(west_grade)
                    )
            ...
            if aps_grade:
                ...
                if aps_normal:
                    ...
                    residual_data['aps_premium'] = self._apply_premium_adjustment(
                        aps_normal, 0.08, long_term_premium, str(aps_grade)
                    )
            ...
            if vgs_grade:
                ...
                if vgs_normal:
                    ...
                    # 고잔가 = 일반잔가 + 6% + 장기계약 추가 (등급별 계수 적용)
                    residual_data['vgs_premium'] = self._apply_premium_adjustment(
                        vgs_normal, 0.06, long_term_premium, str(vgs_grade)
                    )
    ...
    def _get_grade_multiplier(self, grade: Optional[str]) -> float:
        """
        등급별 장기 프리미엄 계수 반환

        Args:
            grade: APS/West 등급 (예: SA1, F, B)

        Returns:
            float: 계수 (1.0 = 100%, 0.75 = 75%, 0.5 = 50%)
        """
        if not grade:
            return 1.0
        ...
        # High tier: 100% 적용
        if grade_upper in ['SA1', 'SA', 'A1', 'A']:
            return 1.0

        # Mid tier: 75% 적용
        elif grade_upper in ['B', 'C', 'D', 'E', 'F', 'G', 'H']:
            return 0.75
    ...
    def _apply_premium_adjustment(self, normal_data: Dict, premium_rate: float,
        ...
        Args:
            ...
            grade: 등급 (등급별 계수 적용용, APS/West 등급)
        ...
        # 등급별 장기 프리미엄 계수
        grade_multiplier = self._get_grade_multiplier(grade)

        for period, mileages in normal_data.items():
            premium_data[period] = {}

            # 48/60개월에는 장기 계약 추가 프리미엄 적용 (등급별 계수 반영)
            total_premium = premium_rate
            if period in [48, 60] and long_term_premium > 0:
                total_premium += (long_term_premium * grade_multiplier)
